...
/**
 * Validate → compile (inject company_id) → execute.
 * Second argument is companyId (string) or `{ companyId }` for callers.
 */
export function executeQueryPlan(
  rawPlan: unknown,
  companyIdOrOptions: string | { companyId: string },
): AnalyticsResult {
  const companyId =
  ...
  if (!companyId) {
    throw new Error("executeQueryPlan requires companyId");
  }
